...
def read_data_from_table(table_name, metric_name=None):
    ...
    try:
        ...
        if data:
            ...
            #data_dict = {key: value for d in data_with_columns for key, value in d.items()}
...
def query(metric_name):
    ...
    if table_name and metric_name:
        ...
        if data:
            ...
            #result_dict = {key: value for d in res_json for key, value in d.items()}
            return res_json
        else:
...
#    # TODO: Implement logic to return a list of metric payload options
#    return jsonify({"payload_options": ["option_1", "option_2"]})
